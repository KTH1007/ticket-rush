...
class GlobalExceptionHandler : ResponseEntityExceptionHandler() {
    ...
    fun handleConflict(e: ConflictException): ResponseEntity<ProblemDetail> {
        val problem = problemDetail(HttpStatus.CONFLICT, e.code, e.message ?: e.code)
        return ResponseEntity.status(HttpStatus.CONFLICT).body(problem)
    }

    @ExceptionHandler(ForbiddenException::class)
    fun handleForbidden(e: ForbiddenException): ResponseEntity<ProblemDetail> {
        val problem = problemDetail(HttpStatus.FORBIDDEN, e.code, e.message ?: e.code)
        return ResponseEntity.status(HttpStatus.FORBIDDEN).body(problem)
    }

    @ExceptionHandler(TooManyRequestsException::class)
    fun handleTooManyRequests(e: TooManyRequestsException): ResponseEntity<ProblemDetail> {
        val problem = problemDetail(HttpStatus.TOO_MANY_REQUESTS, e.code, e.message ?: e.code)
